...
test_set = {
    ...
    "GD_params": {
        "lr": [0.2],
        "beta": [(0.2 - 1) / ((np.sqrt(4 * 0.2**2 + 1) + 1) / 2)],
    },
...
runner_results.append(runner.get_result()[0])
plotter = (
    GradientDescentResultPlotter(runner_results)
    .plot_accuracies_over_time()
    .legend_placed("center right")
    .with_result_labelled(["Momentum", "NAG", "No acceleration"])
)
plotter.plot()
