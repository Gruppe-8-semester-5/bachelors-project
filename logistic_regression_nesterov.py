...
test_set = {
    'w0': start_weight,
    'GD_params': {'w0': start_weight, 'L':[0.1, 0.2, 0.5],'mu':[0.1, 0.2, 0.5], 'alpha': [1/100], 'beta': [0.9]},
    # 'GD_params': {'L': [0.01], 'w0': w0},
    'alg': [Nesterov_acceleration],
    'model': multinomial_logistic_regression,
    'max_iter': iterations,
...
runner_results.append(runner.get_result()[6])
runner_results.append(runner.get_result()[7])
runner_results.append(runner.get_result()[8])
...
GradientDescentResultPlotter(runner_results).plot_accuracies_over_time().hide_y_axis().legend_placed("center right").with_labels(["1","2","3","4","5","6","7","8","9"]).plot()
