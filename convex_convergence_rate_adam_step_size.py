...
constants_normal = [0.1, 0.5, 1, 2, 4]
step_sizes = [used * x for x in constants_normal]
test_set = {
    "w0": w0,
...
for i, step_size in enumerate(step_sizes):
    ...
    plt.plot(x_values, result.get_losses_over_time() - smallest_loss.item(), label=f"\u03B1={constants_normal[i]}/L")
plt.legend(loc="center right")
plt.yscale('log')
plt.ylim(0.06, 1)
save("convergence_convex_adam_step_sizes")
